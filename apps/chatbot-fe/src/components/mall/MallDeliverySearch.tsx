...
export const MallDeliverySearch: React.FC = () => {
  ...
  const fetchProducts = async () => {
    ...
    try {
      ...
      // Use the updated API endpoint for mall delivery products
      const response = await axios.get('http://localhost:8000/api/mobile/mall-delivery/products', {
        params: { location: selectedLocation },
        signal: controller.signal
      });
      
      // If API fails, use mock data
      if (!response.data || response.data.status !== 'success' || !response.data.services) {
        ...
        const productData = response.data.services.map((item: any) => ({
          id: item.id.toString(),
          name: item.name,
          price: item.price,
          image: item.images && item.images.length > 0 ? item.images[0] : 'https://images.unsplash.com/photo-1513104890138-7c749659a591',
          description: item.description || item.short_description,
          location: (item.tags && item.tags.length > 0) ? item.tags[0] : 'Unknown'
        }));
