...
async def chat(chat_message: ChatMessage):
    try:
        ...
        # Get products from different categories with location filter if specified
        location = chat_message.location.city if chat_message.location else None
        
        kids_activities = woocommerce_service.get_kids_activities(location)
        bio_food = woocommerce_service.get_bio_food(location)
        antipasti = woocommerce_service.get_antipasti(location)
        pet_care = woocommerce_service.get_pet_care(location)
        allergy_products = woocommerce_service.get_allergy_products(location)

        # Extract product details from each category
        kids_activities_data = kids_activities.get("activities", []) if kids_activities["status"] == "success" else []
        bio_food_data = bio_food.get("food_items", []) if bio_food["status"] == "success" else []
        antipasti_data = antipasti.get("antipasti_items", []) if antipasti["status"] == "success" else []
        pet_care_data = pet_care.get("pet_items", []) if pet_care["status"] == "success" else []
        allergy_products_data = allergy_products.get("allergy_items", []) if allergy_products["status"] == "success" else []
        
        # Format product details with prices
        def format_product(p):
            return f"{p['name']} - {p['price']} RON"
        
        # Prepare context for AI
        context = {
            "locations": location_names,
            "kids_activities": [format_product(p) for p in kids_activities_data],
            "bio_food": [format_product(p) for p in bio_food_data],
            "antipasti": [format_product(p) for p in antipasti_data],
            "pet_care": [format_product(p) for p in pet_care_data],
            "allergy_products": [format_product(p) for p in allergy_products_data],
...
# Test endpoint for WooCommerce API
